		...
		try {
			...
			if (c.equals(Boolean.class) && !(value instanceof Boolean)) {
				if (value instanceof Number) {
					...
					value = Constants.TRUE.equals(value) || Constants.ONE.equals(value);
				}
			}
			...
			if (value == null && isPrimitive) {
				...
				throw new PropertyAccessException(npe, "NullPointerException occurred while calling", true,
						containerClass, propertyName);
			}
		} catch (InvocationTargetException ite) {
			throw new PropertyAccessException(ite, "Exception occurred inside", true, containerClass, propertyName);
		} catch (IllegalAccessException iae) {
			throw new PropertyAccessException(iae, "IllegalAccessException occurred while calling", true,
					containerClass, propertyName);
			// cannot occur
	...
	private static class SerialForm implements Serializable {
		...
		private Method resolveMethod() {
			try {
				...
				throw new PropertyAccessSerializationException("Unable to resolve setter method on deserialization : "
						+ declaringClass.getName() + "#" + methodName + "(" + argumentType.getName() + ")");
			}
		}
	}
}
