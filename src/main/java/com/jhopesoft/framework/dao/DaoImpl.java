...
public class DaoImpl extends HibernateDaoSupport implements Dao {
	...
	public <T> List<T> findByProperty(Class<T> t, Object... obj) {
		if (obj != null && obj.length % Constants.INT_2 != 0) {
		...
		if (!CommonUtils.isEmpty(obj)) {
			...
			int j = 0;
			for (int i = 0; i < obj.length; i += Constants.INT_2) {
	...
	private List<Map<String, Object>> arrayToMap(String[] fields, List<Object[]> resultList) {
		...
		if (FDatasource.ORACLE.equals(databasename)) {
					...
					for (int j = 0; j < fields.length; j++) {
						...
						if (datas[j] != null && Proxy.isProxyClass(datas[j].getClass())) {
							if (Proxy.getInvocationHandler(datas[j]) instanceof SerializableBlobProxy) {
								SerializableBlobProxy proxy = (SerializableBlobProxy) Proxy
										.getInvocationHandler(datas[j]);
								...
								throw new RuntimeException("Proxy的类型为："
										+ Proxy.getInvocationHandler(datas[j].getClass().getName()) + ",请在上面加入处理代码。");
							}
						} else {
	 ...
	 * 
	 * @param hql     查询语句
	 * @param params1 数组参数 例如： user.name=?
	 * @param params2 枚举参数 例如：user.name=:name
	 * @return
	 */
