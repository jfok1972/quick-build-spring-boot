...
public abstract class SqlMapperAdapter {
	 ...
	 * @param tableName 表名称
	 * @param field     字段名称
	 * @param where     条件名称,通过当前父节点下最大的子节点+1继续生成新的levelid
	 * @param pwhere    查询父节点,通过父节点拼接子节点的levelid
	 * @return
	 */
	...
	public void updateLevel(String tablename, String levelfield, String idfield, String parentfield, String v,
		...
		String upSql = "update " + tablename + " set " + levelfield + " = ?0 where " + idfield + " = ?1 ";
	...
	public SqlFunction getSf() {
		if (sf == null) {
			sf = JdbcAdapterFactory.getJdbcAdapter(dao.getDatabaseType());
		}
		return sf;
	}
