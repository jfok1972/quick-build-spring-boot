...
public class ProjectUtils {
	 ...
	 * 文件资源bundle
	 */
	private final static Map<String, ResourceBundle> BUNDLEMAP = new HashMap<String, ResourceBundle>(0);
	...
	public static Map<String, EntityTypeImpl<?>> getEntityMap(Dao dao) {
		if (entityTypeMap == null) {
			entityTypeMap = new HashMap<String, EntityTypeImpl<?>>(0);
	 ...
	 * 执行注解的对象方法
	 * 
	 * @param annClazz   注解类 例如: Module.class
	 * @param e          注解泛型标识 例如: Module.Type.saveOrUpdate
	 * @param objectname 实体对象名称: 例如:FUser
	 * @param adapter
	 * @param params     参数,按照参数名称注入
	 */
	public static void invokeLogic(Class<? extends Annotation> annClazz, Enum<?> e, String objectname,
		...
		for (String clazzName : beanMap.keySet()) {
			...
			if (!objectname.equals(name) && !name.toLowerCase().equals(Constants.ALL)) {
				continue;
			}
	...
	public static void sendError(ErrorType type, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			if (head == null) {
				...
				response.setStatus(type.getValue());
				// 原来返回错误是下面这个函数，结果 spring boot 会全部转换成404，用上面的即可。
	...
	public static ResultBean getErrorMassage(Exception e, FDataobject dataobject, Dao dao, SqlFunction sf) {
		...
		// 更改过数据库的唯一键值或索引后，需要重启服务器才能生效
		if (e.getCause() instanceof ConstraintViolationException) {
			ConstraintViolationException cve = (ConstraintViolationException) e.getCause();
			if (Constants.SQLSTATE_23000.equals(cve.getSQLState())) {
				String message = cve.getCause().getMessage();
				checkDuplicateKey(message, dataobject, sf, dataMap, resultMap);
			}
		}
		if (resultMap.isEmpty()) {
			// jdbcTemplate 的操作产生的异常和上面的不一样
			checkDuplicateKey(CommonUtils.getThrowableOriginalMessage(e), dataobject, sf, dataMap, resultMap);
			if (resultMap.isEmpty()) {
				...
				if (throwable != null) {
					...
					// sql server 抛出异常：
					// raisError('{totalPrice:"总金额不能为负数！",aaa:"other error"}', 16, 11);
					// 如果返回“{fieldname : "name" , msg : "错误信息" }”,则转为字段
					String errormsg = throwable.getMessage().trim();
					String kh = "{";
					if (errormsg.startsWith(kh)) {
						try {
							JSONObject object = JSONObject.parseObject(errormsg);
							result.setData(object);
						} catch (Exception e1) {
							System.out.println("出错json解析错误:" + errormsg);
						}
					}
					if (result.getData() == null) {
						result.setMessage(
								(StringUtils.isEmpty(result.getMessage()) ? "" : result.getMessage() + ";") + errormsg);
					}
				}
			} else {
	...
	public static void checkDuplicateKey(String message, FDataobject dataobject, SqlFunction sf,
			Map<String, Map<String, Object>> dataMap, Map<String, Object> resultMap) {
		// 需要找到索引名称
		// mysql //Duplicate entry '主键值' for key 'IX_province_name_u'
		// sqlserver ：类型是key
		// 违反了 PRIMARY KEY 约束 'pk_f_organization'。不能在对象 'dbo.f_organization'中插入重复键。
		// 违反了 UNIQUE KEY 约束'IX_f_organization'。不能在对象'dbo.f_organization'中插入重复键。
		// 类型：索引
		// sqlserver //不能在具有唯一索引 'ix_personnelcode' 的对象 'dbo.f_personnel' 中插入重复键的行。
		// oracle //ORA-00001: 违反唯一约束条件 (YZJDPM.PK_F_ORGANIZATION)
		boolean condition = !CommonUtils.isEmpty(message)
				&& (message.startsWith("Duplicate") || message.startsWith("ORA-00001")
						|| message.indexOf("PRIMARY KEY") > -1 || message.indexOf("UNIQUE KEY") > -1)
				|| message.indexOf("不能在具有唯一索引") > -1;
		if (condition) {
			Pattern pattern = Pattern.compile(sf.getDuplicateMessageRegexString());
			Matcher matcher = pattern.matcher(message);
			if (matcher.find()) {
				String value = dataobject.getTitle() + "中已有相同值的记录，不能重复录入！";
				// 第二个符合的值是索引名称
