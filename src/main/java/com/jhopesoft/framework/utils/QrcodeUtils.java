...
public class QrcodeUtils {
  ...
  public static ByteArrayOutputStream encodeImgLogo(InputStream source, InputStream logoImg) {
    ...
    try {
      ...
      int logoWidth = logo.getWidth(null) > target.getWidth() * 2 / 10 ? (target.getWidth() * 2 / 10)
          : logo.getWidth(null);
      int logoHeight = logo.getHeight(null) > target.getHeight() * 2 / 10 ? (target.getHeight() * 2 / 10)
          : logo.getHeight(null);
      // 设置logo图片放置位置
      // 中心
    ...
    try {
      ImageIO.write(target, Constants.JPG, result);
    } catch (IOException e) {
      e.printStackTrace();
    }
    return result;
  ...
  public static ByteArrayOutputStream createQrCode(String contents, int size, InputStream logstream) {
    ByteArrayOutputStream source = writeToStream(contents, Constants.JPG, size);
    if (logstream != null) {
