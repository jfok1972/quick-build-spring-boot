	...
	public static <T> List<T> buildListToTree(List<T> dirs) {
		...
		for (T root : roots) {
			if (root instanceof TreeNode) {
				...
				((Map<String, Object>) root).put(Constants.CHILDREN, findChildren(root, notRoots));
			}
		}
		return roots;
	}
	...
	private static <T> List<T> findRoots(List<T> allTreeNodes) {
		...
		for (T t : allTreeNodes) {
			...
			if (t instanceof TreeNode) {
				...
				node.setParentId(CommonUtils.isEmpty(parentId) ? Constants.ZEROZERO : parentId);
				...
				node.put(parentidkey, CommonUtils.isEmpty(parentId) ? Constants.ZEROZERO : parentId);
				for (T t1 : allTreeNodes) {
					Map<String, Object> comparedOne = (Map<String, Object>) t1;
					Object id = CommonUtils.isEmpty(comparedOne.get(Constants.ID)) ? comparedOne.get(menuidkey)
							: comparedOne.get(Constants.ID);
	...
	private static <T> List<T> findChildren(T root, List<T> allTreeNodes) {
		...
		for (T t : allTreeNodes) {
			if (t instanceof TreeNode) {
				...
				Object id = CommonUtils.isEmpty(root1.get(Constants.ID)) ? root1.get(menuidkey)
						: root1.get(Constants.ID);
				if (comparedOne.get(parentidkey).equals(id)) {
					comparedOne.put(Constants.LEVEL, (int) root1.get(Constants.LEVEL) + 1);
					children.add((T) comparedOne);
				}
			}
		}
		...
		for (T t : children) {
			if (t instanceof TreeNode) {
				...
				child.put(Constants.CHILDREN, tmpChildren);
			}
		}
		sort(children);
		return children;
