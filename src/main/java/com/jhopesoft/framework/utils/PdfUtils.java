...
public class PdfUtils {
  ...
  private static boolean convert(Object input, DocumentFormat inputFormat, Object output, DocumentFormat outputFormat) {
    ...
    try {
      ...
      if (output instanceof File) {
        ...
        String filename = outputFormat == null ? Constants.PDF : outputFormat.getExtension();
  ...
  public static void initOpenOffice() throws IOException, OfficeException {
    logger.info(String.format("open offcie start: %s", DateUtils.format(Constants.DATE_TIME_FORMAT)));
    ...
    if (!CommonUtils.isEmpty(home)) {
      System.setProperty("office.home", home);
    }
    DefaultOfficeManagerBuilder builder = new DefaultOfficeManagerBuilder();
    if (!CommonUtils.isEmpty(profile)) {
      builder.setTemplateProfileDir(new File(profile));
    }
    if (!CommonUtils.isEmpty(port)) {
      // 启动多个服务，解决并发问题。
      String[] ports = port.trim().split(Constants.COMMA);
