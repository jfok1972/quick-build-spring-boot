...
public class HierarchyIDPIDUtils {
	...
	public static String _getIDPIDExpression(int level, String fieldexpression, List<HierarchyIDPID> hierarchyidpids) {
		...
		List<HierarchyIDPID> levelHierarchyIDPIDs = new ArrayList<HierarchyIDPID>(0);
		// 找到所有该级别的记录
	...
	private static String genCaseWhen(String fieldexpression, List<HierarchyIDPID> keyvalues) {
		...
		return " case when " + fieldexpression + getin + " then " + OperateUtils.translateValue(record.getId())
				+ " else " + (keyvalues.size() > 0 ? genCaseWhen(fieldexpression, keyvalues) : "'" + UNDEFINED + "'")
				+ " end ";
	}
	...
	private static String getIn(String[] values, String fn) {
		...
		return OperateUtils.valueChangeToInString(String.join(Constants.COMMA, values), fn);
