...
public class SessionManage implements HttpSessionListener, ServletContextListener {
  ...
  public void sessionDestroyed(HttpSessionEvent event) {
    ...
    if (session.getAttribute(Globals.LOGINLOG) != null) {
      ...
      Object forceLogout = session.getAttribute(Globals.FORCE_LOGOUT);
      if (loginLogic != null) {
        loginLogic.writeLogout((FUserloginlog) event.getSession().getAttribute(Globals.LOGINLOG),
            forceLogout != null ? forceLogout.toString() : "超时登出");
      }
    }
  }
  ...
  public void contextDestroyed(ServletContextEvent arg0) {
    System.out.println("应用服务器已停止，最好把所有的session都写入logout");
  }
