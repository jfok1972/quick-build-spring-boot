 ...
 *
 */
public class ExcelExportPOI_NOCACHE {
	/** 超出了这个记录数就不执行自动size */
	...
	/** 树形最大有几层，0-1-2，2表示3层，在column字段列表里每个字段都有level值，而记录最底层的没有level值 */
	...
	public OutputStream GenExcel(FDataobject module, String title, List<UserDefineFilter> conditionList,
		...
		if (group != null && group.getProperty() != null && group.getProperty().length() > 0) {
			// 如果是manytoone的idfield,改成 manytoone的namefield
		...
		for (ExcelColumn column : dataIndexColumns) {
			...
			if (column.getGridFieldId() != null || column.getFormFieldId() != null) {
				...
				if (column.getGridFieldId() != null) {
					...
					// 树形结构的excel导出用的是formSchemedetail
				...
				info.setFieldname(column.getDataIndex());
				info.setFieldtype("Integer");
				if ("actProcState".equals(column.getDataIndex())) {
		...
		try {
			...
			if (!setting.isSumless()) {
				calcHjAndWirte(fu_value, sheet, totalrow, groupColumnCount, gridFieldInfos);
			}
			// recordFirstCol,加入层次的折叠
			for (int i = 0; i < fu_value.size(); i++) {
				Map<String, Object> v = (Map<String, Object>) fu_value.get(i);
			...
			// 如果有__level__ ,把比level值高的要汇总的字段都加入 表达式
			for (int i = 0; i < fu_value.size(); i++) {
				Map<String, Object> v = (Map<String, Object>) fu_value.get(i);
			...
			// 自动适应后每列不大于50字符,最小不小于5个字符，隐藏的除外
			...
			// ColumnWidth > 22000 ,A4就放不下了
			int sumWidth = 0;
			for (short column = 0; column <= maxY; column++) {
				sumWidth += sheet.getColumnWidth(column);
			}
			// 如果总宽度太小，那么把宽度加大一点，不然有点难看
			if (sumWidth < 80 * 256) {
				for (short column = 0; column <= maxY; column++) {
			...
			if ("pageautofit".equals(setting.getPagesize())) {
			...
			// 冻结表头行
			sheet.createFreezePane(0, x, 0, x);
			// 设置重复打印的列头
			sheet.setRepeatingRows(new CellRangeAddress(0, x - 1, 0, 0));
	...
	@SuppressWarnings("unchecked")
	public void calcSubHjAndWirte(List<?> fuValue, Sheet sheet, int x, int groupcolumns, String groupfieldname,
		...
		try {
			...
			int count = 0;
			// 计算小计
			for (int i = 0; i < fuValue.size(); i++) {
				Map<String, Object> v = (Map<String, Object>) fuValue.get(i);
	...
	public ExcelExportPOI_NOCACHE(ExcelExportSetting setting, boolean toPDF) {
