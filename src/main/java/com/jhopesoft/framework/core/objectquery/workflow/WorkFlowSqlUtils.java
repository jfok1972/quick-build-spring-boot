...
public class WorkFlowSqlUtils {
	...
	public static String getLeftOuterJoin(BaseModule baseModule) {
		...
		String leftouter;
		if (FDatasource.MYSQL.equals(Local.getDao().getDatabaseType().toLowerCase())) {
			leftouter = MYSQLLEFTOUTERJOIN;
		} else if (FDatasource.ORACLE.equals(Local.getDao().getDatabaseType().toLowerCase())) {
			leftouter = ORACLELEFTOUTERJOIN;
		} else {
			leftouter = MSSQLLEFTOUTERJOIN;
		}
