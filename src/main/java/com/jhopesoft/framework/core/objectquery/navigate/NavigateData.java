...
public class NavigateData {
	...
	public NavigateData(JSONObject jsonObject, int level) {
		...
		if (this.jsonObject.containsKey("name_" + level)) { // manytoone，
	...
	public JSONObject genJsonObject(SqlGenerate sqlGenerate, List<FovGridnavigateschemedetail> navigateSchemeDetails) {
		...
		if (key == null) {
			record.put(Constants.OPERATOR, "is null");
			record.put("fieldvalue", "_null_");
			record.put(Constants.TEXT, "未定义");
			record.put("iconCls", "x-fa fa-chain-broken");
		} else {
			if (schemeDetail.getIconcls() != null) {
				record.put("iconCls", schemeDetail.getIconcls());
			}
			record.put(Constants.OPERATOR, this.operator);
			record.put("fieldvalue", key);
			if (schemeDetail.getFNumbergroup() == null)
				record.put(Constants.TEXT, name == null ? key : name);
			else {
				record.put(Constants.TEXT, schemeDetail.getFNumbergroup().getDetailTitle(Integer.parseInt(key) - 1000));
			}
		}

		record.put(Constants.COUNT, count);
		...
		record.put(Constants.MODULE_NAME, schemeDetail._getFactAheadPath() == null ? baseModule.getModule().getObjectname()
		...
		if (children != null) {
			record.put("expanded",
					"startwith".equals(record.get(Constants.OPERATOR)) && level == 1 ? true : !schemeDetail.getCollapsed());
			JSONArray childrens = new JSONArray();
			for (NavigateData data : children) {
				childrens.add(data.genJsonObject(sqlGenerate, navigateSchemeDetails));
			}
			record.put(Constants.CHILDREN, childrens);
		}
		return record;
	}
