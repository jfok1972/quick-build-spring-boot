...
public class SqlModuleFilter implements Serializable {
	...
	public String getAs() {
		if (primarykey.indexOf(Constants.SPACE) != -1 || primarykey.indexOf(Constants.DOT) != -1) {
			return "";
		} else {
			return tableAsName + Constants.DOT;
		}
	}
	...
	private String getThisFilterSql() {
		...
		if (equalsValue == null) {
			equalsValue = Constants.NULL;
		}
		if (Constants.TRUE.equals(equalsValue)) {
			equalsValue = "1";
		}
		if (Constants.FALSE.equals(equalsValue)) {
			equalsValue = "0";
		}
		if (Constants.NULL.equals(equalsValue)) {
			return "(" + getAs() + primarykey + " is null )";
		}
		if (Constants.NOTNULL.equals(equalsValue)) {
			return "(" + getAs() + primarykey + " is not null )";
		}
		equalsValue = equalsValue.replaceAll("'", "");
		if (equalsMethod != null) {
			if (EQUALSMETHOD_YEAR.equals(equalsMethod)) {
				return "( year(" + getAs() + primarykey + ")=" + equalsValue + ")";
			} else if (EQUALSMETHOD_YEARMONTH.equals(equalsMethod)) {
				return "( year(" + getAs() + primarykey + ")=" + equalsValue.substring(0, 4) + " and month(" + getAs()
						+ primarykey + ")=" + equalsValue.substring(5, 7) + ")";
			} else if (EQUALSMETHOD_YEARSEASON.equals(equalsMethod)) {
