...
public class SqlGenerate {
	...
	private static final Log log = LogFactory.getLog(SqlGenerate.class);
	...
	private boolean addShortNameField = false;
	...
	private Set<SqlField> aggregatefields = new LinkedHashSet<SqlField>();
	...
	public SqlGenerate pretreatment() {
		...
		if (addShortNameField){
			SqlFieldUtils.addShortNameField(baseModule, selectfields);
		}
		if (addPidField) {
		...
		if (dataMiningGroupField != null) {
			Set<SqlField> groupfields = SqlFieldUtils.getSqlFieldFromParentChildField(baseModule, dataMiningGroupField,
					null, false);
			for (SqlField field : groupfields) {
				...
				if (dataMiningGroupFieldLeveltype != null) {
					...
					if (dataMiningGroupFieldLeveltype.length() <= 2) {
						field.setSqlstatment(dataobject._getLevelExpression(
								Integer.parseInt(dataMiningGroupFieldLeveltype), field.getSqlstatment()));
					}
				}
	...
	public String generateSelect() {
		...
		for (String field : fields) {
			...
			sql.append((field == fields.get(fields.size() - 1) ? "" : Constants.COMMA) + CR);
		}
		sql.append(" from " + CR);
		for (String from : FromGenerate.generateFrom(this, baseModule, false,
				(sql.toString().indexOf("act_") >= 0 || hasActFieldinCondition()))) {
	...
	public String generateSelectCount() {
		...
		sql.append(" from " + CR);
		for (String from : FromGenerate.generateFrom(this, baseModule, true, hasActFieldinCondition())) {
			sql.append(TAB + from + CR);
		}
		if (wheres.size() > 0) {
			sql.append(" where " + CR);
	...
	public String generateSelectRemoteTotal() {
		aggregatefields = new LinkedHashSet<SqlField>();
		StringBuilder sql = new StringBuilder("select ");
		List<String> scales = new ArrayList<String>();
		List<String> fields = FieldGenerate.generateSelectRemoteTotalFields(baseModule, selectfields, aggregatefields,
				scales);
		for (String field : fields) {
			sql.append(TAB + field);
			sql.append((field == fields.get(fields.size() - 1) ? "" : Constants.COMMA) + CR);
		...
		for (String scale : scales) {
			sumsql.append(TAB + "sum(s_." + scale + ") as " + scale);
			sumsql.append((scale == scales.get(scales.size() - 1) ? "" : Constants.COMMA) + CR);
		}
		sumsql.append(" from ( " + sql.toString() + " ) s_");
		String asql = Local.getBusinessDao().getSf().adjustSqlstatment(sumsql.toString());
		log.info(asql);
		return asql;
	}
	...
	public String[] getTotalFieldNames() {
		...
		int i = 0;
		for (SqlField field : aggregatefields) {
			result[i++] = field.getFieldname();
		}
		return result;
	}
	...
	public Set<SqlField> getAggregatefields() {
		return aggregatefields;
	}

	public void setAggregatefields(Set<SqlField> aggregatefields) {
	...
	public boolean isAddShortNameField() {
		return addShortNameField;
	}

	public void setAddShortNameField(boolean addShortNameField) {
		this.addShortNameField = addShortNameField;
	}
	...
	public void onlyAddIdshortNameFields() {
		addIdField = true;
		addShortNameField = true;
		addPidField = true;
		addBaseField = false;
		addAllGridScheme = false;
		addAllFormScheme = false;
