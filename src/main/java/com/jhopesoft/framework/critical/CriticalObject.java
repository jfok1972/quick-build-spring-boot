...
public class CriticalObject {
	...
	public CriticalObject(HttpServletRequest request, HttpServletResponse response) {
		...
		this.dao = AppContextAware.getApplicationContext().getBean(Dao.class);
	...
	public Dao getBusinessDaoFromParameter(HttpServletRequest request) {
		...
		String objectid = request.getParameter(Constants.MODULE_NAME);
		if (objectid == null) {
			// antd ,的 request 需要加这个，不然无法取得multipart/form-data参数
			objectid = request.getParameter("moduleName_");
		}
		if (objectid == null) {
			objectid = request.getParameter(Constants.OBJECTID);
		}
		if (objectid == null) {
