		...
		if (beFilter) {
			...
			if (bean == null || CommonUtils.isEmpty(bean.getUserid())) {
				...
				if (!CommonUtils.isEmpty(sessionid)) {
					HttpSession oldSession = SessionUtils.SESSIONCONTEXT.get(sessionid);
					session.setAttribute(Globals.SYSTEM_USER, oldSession.getAttribute(Globals.SYSTEM_USER));
				} else {
					if (request.getHeader(Constants.ANTD) != null) {
						ProjectUtils.sendError(ErrorType.E401, request, response);
					} else {
