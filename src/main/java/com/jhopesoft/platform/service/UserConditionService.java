...
public class UserConditionService {
  ...
  public UserConditionTreeNode getUserCondition(HttpServletRequest request, String dataObjectId) {
    ...
    UserConditionTreeNode system = new UserConditionTreeNode("系统自定义条件", "system", true, false,
        new ArrayList<UserConditionTreeNode>());
    UserConditionTreeNode owner = new UserConditionTreeNode("我的自定义条件", "owner", true, false,
        new ArrayList<UserConditionTreeNode>());
    UserConditionTreeNode othershare = new UserConditionTreeNode("其他用户分享的自定义条件", "othershare", true, false,
        new ArrayList<UserConditionTreeNode>());
    ...
    if (system.getChildren().size() > 0) {
      result.getChildren().add(system);
    }
    result.getChildren().add(owner);
    if (othershare.getChildren().size() > 0) {
      result.getChildren().add(othershare);
    }
    return result;
  }
  ...
  public ActionResult updateSchemeDetails(String dataObjectId, String conditionid, String schemename,
    ...
    UserConditionTreeNode node = new UserConditionTreeNode(condition.getTitle(), condition.getConditionid(), null, true,
        null);
    node.setIsshare(condition.getIsshare());
    node.setIsshareowner(condition.getIsshareowner());
    result.setTag(node);
  ...
  public ActionResult saveasScheme(HttpServletRequest request, String conditionid) {
    ...
    ActionResult result = new ActionResult();
    UserConditionTreeNode node = new UserConditionTreeNode(saveas.getTitle(), saveas.getConditionid(), null, true,
        null);
  ...
  private void copySchemeDetail(FDataobjectconditiondetail column, FDataobjectconditiondetail pcolumn) {
    for (FDataobjectconditiondetail c : column.getDetails()) {
      FDataobjectconditiondetail newcolumn = new FDataobjectconditiondetail(null,
