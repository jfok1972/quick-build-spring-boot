...
public class WorkFlowDesignService {
	...
	public ActionResult saveDeploy(String workflowid) throws WorkFlowException {
		...
		boolean bpmnfilefound = StringUtils.isNotBlank(workflowdesign.getBpmnxml());
		...
		if (!bpmnfilefound) {
			...
			for (FDataobjectattachment attachment : attachments) {
				...
				} else if (attachment.getSuffixname().equalsIgnoreCase(Constants.PNG)) {
		...
		if (bpmnfilefound) {
			if (StringUtils.isNotBlank(workflowdesign.getBpmnxml())) {
				builder = repositoryService.createDeployment().name(workflowdesign.getName()).addString(
						workflowdesign.getName() + ".bpmn", changeCodeOrNameToId(workflowdesign.getBpmnxml()));
				// 加入图片定义
				// 如果不是必需要就不加了
			} else {
				builder = repositoryService.createDeployment().name(workflowdesign.getName()).addInputStream(
						bpmnattachment.getFilename(),
						changeCodeOrNameToId(attachmentService.getOriginalFileStream(bpmnattachment)));
				if (pngattachment != null) {
					builder.addInputStream(pngattachment.getFilename(),
							attachmentService.getOriginalFileStream(pngattachment));
				}
			}
			...
			if (!definitionEntity.getKey().equals(workflowdesign.getProcDefKey())) {
				throw new WorkFlowException(
						messagehead + "的定义文件中，process 的 id 不是‘" + workflowdesign.getProcDefKey() + "’。");
			}
	...
	public ActionResult setValid(String workflowid) {
		...
		List<FWorkflowdesign> designs = dao.findByProperty(FWorkflowdesign.class, "procDefKey",
				workflowdesign.getProcDefKey());
		for (FWorkflowdesign design : designs) {
			if (workflowid.equals(design.getWorkflowid())) {
				design.setLatestversion(true);
				dao.update(design);
			} else if (BooleanUtils.isTrue(design.getLatestversion())) {
				design.setLatestversion(false);
				dao.update(design);
			}
		}
	...
	private InputStream changeCodeOrNameToId(InputStream is) {
		...
		String result = changeCodeOrNameToId(str);
		try {
	...
	private String changeCodeOrNameToId(String str) {
	...
	private String changeUserCodeOrNameToId(String s) {
		...
		String[] codes = str.split(Constants.COMMA);
		for (int i = 0; i < codes.length; i++) {
			String code = codes[i];
			FUser user = dao.findById(FUser.class, code);
			if (user == null) {
				user = (FUser) dao.findByPropertyFirst(FUser.class, Constants.USERCODE, code);
				if (user == null) {
					user = (FUser) dao.findByPropertyFirst(FUser.class, Constants.USERNAME, code);
				}
				if (user != null) {
		...
		return ahead + "\"" + String.join(Constants.COMMA, codes) + "\"";
	}
	...
	private void addWorkFlowFieldsToObject(FDataobject object) {
		...
		saveField(object, "actCurrentAssignName", "act_procinst.CURRENT_ASSIGN_NAME_", "String", 200, "等待审核人员",
				orderno++, null);
		...
		// 只有当前用户审批的才有值，否则只有 等待审核人员 里面有个人名字
		...
		// 下面这个没用
		// saveField(object, "actCandidateName", "act_task.candidatename", "String",
		// 2000, "可拾取人员名称", orderno++, null);
		...
		saveField(object, "actCompleteTaskInfo", "act_procinst.COMPLETE_TASK_INFO_", "String", 2000, "已完成任务说明",
				orderno++, null);
	...
	private FDataobjectfield saveField(FDataobject dataObject, String fieldname, String formula, String fieldtype,
		...
		dao.save(result);
		dataObject.getFDataobjectfields().add(result);
		return result;
	}
	...
	public String getSvgFromProcDefId(String procDefId) {
		FWorkflowdesign design = dao.findByPropertyFirst(FWorkflowdesign.class, "procDefId", procDefId);
		if (design != null) {
			return design.getBpmnsvg();
		}
		return "";
	}
