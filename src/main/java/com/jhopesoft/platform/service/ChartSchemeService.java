...
public class ChartSchemeService {
	...
	public JSONArray getSchemes(String moduleName, String dataminingschemeid) {
		...
		if (schemes != null) {
			for (FovChartscheme scheme : schemes) {
				...
				object.put(Constants.TEXT, scheme.getTitle());
	...
	public ActionResult getScheme(String schemeid, String viewschemeid, String userfilters) {
		...
		object.put(Constants.OBJECTID,
				scheme.getFDataobject() != null ? scheme.getFDataobject().getObjectid()
						: (scheme.getFDataminingscheme() != null
								? scheme.getFDataminingscheme().getFDataobject().getObjectid()
								: null));
		// 如果是商业数据分析的图表
		if (scheme.getFDataminingscheme() != null)
			object.put(Constants.DATA,
					dataminingDataService.fetchDataminingData(scheme.getFDataminingscheme().getSchemeid(),
							true, viewschemeid, userfilters, null, false));
		else {
			// 是模块的图表,会加入所有的可视记录。
			object.put(Constants.DATA, dataObjectService.fetchDataInner(scheme.getFDataobject().getObjectid(),
