...
public class WorkFlowRuntimeService {
	 ...
	 * 检查流程是否启动，如果启动则不允许删除
	 * 
	 * @param objectName 模块名称,也是proc_def_key
	 * @param id         模块记录id
	 * @return
	 */
	...
	public String getValidProcessInstanceById(String objectName) {
		...
		if (workflowdesigns.size() != 1) {
			...
			if (workflowdesigns.size() == 0) {
				throw new WorkFlowException("模块『" + object.getTitle() + "』没有生效的审批流程定义！");
			} else {
				throw new WorkFlowException("模块『" + object.getTitle() + "』有多个生效的审批流程定义！");
			}
		} else {
	 ...
	 * @param id         模块记录id
	 * @param name       模块记录名称值
	 * @param processInstanceId 指定一个审批流程的id
	 * @return
	 */
	...
	public ResultBean cancelProcessInstance(String objectName, String id, String name) throws WorkFlowException {
		...
		dataObjectService.saveOperateLog(dataObject, procinst.getBusinessKey(), procinst.getBusinessName(), "取消审批信息",
				null);
	 ...
	 * @param id
	 * @param name
	 * @param outgoingid   执行的连线的id
	 * @param outgoingname 执行的连线的name
	 * @param taskId
	 * @param moduledata   业务系统的修改数据
	 * @return
	...
	public void getInstanceDiagram(String processInstanceId) throws IOException {
		...
		InputStream imageStream = diagramGenerator.generateDiagram(bpmnModel, Constants.PNG, highLightedActivitis,
				highLightedFlows, "宋体", "宋体", null, null, 2.0);
		// 单独返回流程图，不高亮显示
		// 输出资源内容到相应对象
