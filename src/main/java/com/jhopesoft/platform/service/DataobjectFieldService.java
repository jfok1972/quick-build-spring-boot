...
public class DataobjectFieldService {
	 ...
	 * 
	 * @param fieldid1       主表字段ID
	 * @param fieldid2       从表字段ID
	 * @param linkedobjectid 中间关联表实体对象ID
	 * @param userid         创建者ID
	 * @return
	 */
	public ActionResult createManyToManyField(String fieldid1, String fieldid2, String linkedobjectid, String userid) {
		...
		field1.setCreatedate(new Timestamp(System.currentTimeMillis()));
		dao.save(field1);
		...
		field2.setCreatedate(new Timestamp(System.currentTimeMillis()));
	...
	public ActionResult createOneToManyField(String fieldid) {
		...
		onetomanyfield.setFieldahead(manyobject.getObjectname() + Constants.DOTWITHDOT + objectfield.getFieldname());
	...
	public List<ValueText> getFieldComboData(String fieldid) {
		...
		if (field != null) {
			if (field._isManyToOne()) {
				...
				return dictionaryService.getDictionaryComboData(field.getFDictionary().getDictionaryid(),
						Local.getRequest());
			} else {
				if (field.getFDataobject().getPrimarykey().equals(field.getFieldname())) {
					// 如果是一个模块的主键，那么也加入所有的combodata
					return dataObjectService.fetchModuleComboData(field.getFDataobject().getObjectname(), false, null,
							null);
				}
			}
		}
	...
	public ActionResult testAdditionField(String additionFieldId) {
		...
		SqlField sqlfield = new SqlField("testfield", SqlFieldUtils.generateSqlFormJsonFieldString(
				generate.getBaseModule(), null, field._getConditionExpression(), false), null);
