...
public class ModuleHierarchyService {
	...
	public JSONObject getModuleHierarchTree(String moduleName, Boolean onlyParentModule, Boolean onlyChildModule,
		...
		root.put(Constants.TEXT, "Root");
		root.put("expanded", true);

		JSONObject moduleObject = new JSONObject();
		moduleObject.put(Constants.TEXT, "<span style='color:blue;'>基准模块：" + module.getTitle() + "</span>");
		...
		boolean cond = (onlyChildModule || onlyParentModule) && !enableBaseModule;
		if (cond) {
			moduleObject.put("disabled", true);
		}
		moduleObject.put(Constants.CHILDREN, pandc);

		JSONArray rootChildren = new JSONArray();
		rootChildren.add(moduleObject);
		root.put(Constants.CHILDREN, rootChildren);
	...
	public JSONObject getModuleHierarchyParentTree(FDataobject module, BaseModule baseModule,
		...
		parentRoot.put(Constants.TEXT, "<span style='color:red;'>父模块</span>");
		...
		Integer[] deep = { 0 };
		parentRoot.put(Constants.CHILDREN, getParentModule(baseModule.getParents(), deep, 1));
	...
	public JSONObject getModuleHierarchyChildTree(FDataobject module, BaseModule baseModule,
		...
		childRoot.put(Constants.TEXT, "<span style='color:red;'>子模块</span>");
		...
		Integer[] deep = { 0 };
		childRoot.put(Constants.CHILDREN, getChildModule(baseModule.getChilds(), deep, 1));
	...
	private JSONArray getChildModule(Map<String, ChildModule> childs, Integer[] deep, int nowdeep) {
		...
		if (childs.size() > 0) {
			...
			for (String childModuleName : childs.keySet()) {
				...
				child.put(Constants.TEXT, childModule.getNamePath());
				...
				if (childModule.getChilds().size() > 0) {
					...
					child.put(Constants.CHILDREN, pms);
					Integer c = 0;
					for (int i = 0; i < pms.size(); i++) {
						c += ((JSONObject) pms.get(i)).getInteger("cols");
					}
	...
	private void addModulePropertyToJson(FDataobject module, JSONObject object) {
		...
		object.put(Constants.MODULE_NAME, module.getObjectname());
	 ...
	 * 
	 * @param moduleName
	 * @param isChildModule 是否选择的是子模块，如果是的话，只能返回聚合字段
	 * @param modulePath    当前子模块到基准模块的路径
	 * @param addcheck      是否加入check标记
	 * @param request
	 * @return jsonarray
	 */
	public JSONArray getModuleFieldsForSelect(String moduleName, Boolean isChildModule, String modulePath,
		...
		for (FDataobjectfield field : module.getFDataobjectfields()) {
			...
			if (BooleanUtils.isNotTrue(field.getIsdisable()) && BooleanUtils.isNotTrue(field.getUserdisable())) {
				boolean cond = isChildModule
						&& (field.getAllowaggregate() == null || field.getAllowaggregate() == false)
				...
				if (isChildModule) {
					f.put("expanded", true);
					f.put(Constants.CHILDREN, getAggregationItems(field, itemId, modulePath, addcheck));
				}
	...
	public void getAggrationItem(FDataobjectfield field, FieldAggregationType type, String itemId, String fieldahead,
		...
		object.put(Constants.TEXT, FieldAggregationType.AGGREGATION.get(type));
		...
		if (fieldahead == null || fieldahead.length() == 0 || fieldahead.indexOf(Constants.DOTWITHDOT) > 0) {
			// 只有本模块字段的子模块的加入模块筛选条件，父模块的不加入（因为没法处理）
			for (FDataobjectcondition condition : dataobject.getFDataobjectconditions()) {
				object = new JSONObject();
				object.put(Constants.TEXT, FieldAggregationType.AGGREGATION.get(type) + "--" + condition.getTitle());
