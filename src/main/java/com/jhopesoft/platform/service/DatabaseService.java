...
import com.jhopesoft.framework.utils.TableBeanGenerator;
...
public class DatabaseService extends SqlMapperAdapter {
	public JSONArray getSchemas() throws SQLException {
		...
		object.put(Constants.TEXT, "默认数据库");
		result.add(object);
		...
		for (String string : getSf().getSchemas(dao)) {
			...
			object.put(Constants.TEXT, string);
			result.add(object);
		}
		return result;
	}
	...
	public TreeValueText getNotImportTableAndViews(String schema) throws SQLException {
		...
		for (String table : tables) {
			...
			if (!found) {
				String comment = getSf().getTableComment(dao, schema, table);
	...
	public FDataobject importTableOrView(String schema, String tablename, String title, String namefield,
		...
		ActionResult result = importTableOrView(schema, tablename, title, namefield, null, "[]", addtoadmin, addtomenu,
				pmenu, params);
	...
	public ActionResult importTableOrView(String schema, String tablename, String title, String namefield,
			String objectgroup, String fields, boolean addtoadmin, boolean addtomenu, FCompanymenu pmenu,
		...
		namefield = CommonUtils.isEmpty(namefield) ? object.getPrimarykey()
				: CamelCaseUtils.getEntityBeanField(namefield);
		object.setNamefield(namefield);
		object.setHasenable(true);
		object.setHasbrowse(true);
		object.setHasinsert(object.getClassname() != null);
		object.setHasedit(object.getClassname() != null);
		object.setHasdelete(object.getClassname() != null);
		object.setSelectedmode("20");
		...
		if (Local.getUserBean() == null) {
			userid = dao.findByPropertyFirst(FUser.class, Constants.USERCODE, Constants.ADMIN).getUserid();
		} else {
		...
		if (companymodulegroup == null) {
			// 放到 公司模块中
			companymodulegroup = dao.findByPropertyFirst(FCompanymodulegroup.class, Constants.COMPANYID,
		...
		// 菜单里
		if (pmenu == null) {
			pmenu = dao.findByPropertyFirst(FCompanymenu.class, Constants.COMPANYID, Constants.ZEROZERO, "menuname",
					groupname);
			if (pmenu == null) {
		...
		Map<String, FDataobjectfield> fieldMap = new HashMap<String, FDataobjectfield>(0);
		...
		for (int i = 0; i < tablebean.getFields().size(); i++) {
			...
			for (int ii = 0; ii < fieldarray.size(); ii++) {
				JSONObject f = fieldarray.getJSONObject(ii);
				if (f.getString(Constants.NAME) != null && f.getString(Constants.NAME).equals(field.getFieldname())) {
					String t = f.getString(Constants.TITLE);
			...
			FDataobjectfield objectfield = getFDataobjectfield(fieldMap, schema, tablename, object, group, field,
					userid, i, objectgroup);
			...
			if (BooleanUtils.isNotTrue(objectfield.getIsdisable())) {
				boolean cond = ((fn.equals(object.getPrimarykey()) && (objectfield.getFieldlen() >= 32)
						&& (!"orgid".equals(fn))) || fn.equals(object.getParentkey()));
				if (!cond) {
					if (!(Constants.CREATER.equals(fn) || Constants.CREATEDATE.equalsIgnoreCase(fn)
							|| Constants.LASTMODIFIER.equalsIgnoreCase(fn)
							|| Constants.LASTMODIFYDATE.equalsIgnoreCase(fn))) {
						// 插入列表(Grid)方案列表
	...
	public ActionResult refreshTableFields(String objectid) throws IllegalAccessException, InvocationTargetException {
		...
		for (int i = 0; i < tablebean.getFields().size(); i++) {
			...
			count++;
			msg = msg + objectfield.getFieldtitle() + Constants.COMMA;
		}
	...
	public FDataobjectfield getFDataobjectfield(Map<String, FDataobjectfield> fieldMap, String schema, String tablename,
		...
		if ("orderno".equalsIgnoreCase(objectfield.getFieldname())) {
		...
		if (StringUtils.isNotBlank(objectfield.getFieldrelation())) {
			if (StringUtils.endsWithIgnoreCase(objectfield.getFieldtitle(), Constants.ID)) {
				objectfield.setFieldtitle(
						objectfield.getFieldtitle().substring(0, objectfield.getFieldtitle().length() - 2));
			}
			FDataobject joinedobject = dao.findByPropertyFirst(FDataobject.class, "lower(tablename)",
				...
				if (analysis) {
					...
					if (joinedobject.getPrimarykey().equals(objectfield.getFieldname())) {
						...
						if (field.isMultiple()) {
							...
							objectfield.setFieldname(joinedobject.getObjectname() + "By"
									+ CamelCaseUtils.firstCharacterUpperCase(fieldname));
						} else {
							objectfield.setFieldname(joinedobject.getObjectname());
						}
					}
	...
	/**
	 * 打包下载当前选中数据库的Java Bean文件。
	 * 
	 * @param schema
	 * @throws IOException
	 */
	public void downLoadBeanFiles(String schemeName) throws IOException {
		boolean addManyToOne = true;
		Map<String, TableBeanGenerator> classBeans = new HashMap<String, TableBeanGenerator>(0);
		List<String> tables = dao.getSf().getTables(dao, schemeName);
		Map<String, TableBean> tableBeanMap = new HashMap<String, TableBean>(0);
		tables.forEach(tableName -> {
			tableBeanMap.put(tableName, genTableBean(schemeName, tableName));
		});
		// 把所有manytoone字段，加上one端的主键值在by5中，用于生成many端的字段名
		tableBeanMap.forEach((tablename, tableBean) -> {
			tableBean.getFields().forEach(field -> {
				if (StringUtils.isNotBlank(field.getJoincolumnname())) {
					TableBean oneSide = tableBeanMap.get(field.getJointable());
					field.setBy5(oneSide.getPrimarykey());
				}
			});
		});
		FSysteminfo systeminfo = dao.findAll(FSysteminfo.class).get(0);
		tableBeanMap.forEach((tablename, tableBean) -> {
			classBeans.put(tablename,
					new TableBeanGenerator(systeminfo.getSystemkey(), schemeName, tableBean, addManyToOne));
		});
		if (addManyToOne) {
			classBeans.forEach((tablename, classBean) -> {
				classBean.getTableBean().getFields().forEach(field -> {
					if (StringUtils.isNotBlank(field.getJoincolumnname())) {
						TableBeanGenerator oneSide = classBeans.get(field.getJointable());
						// 写入onetomany
						oneSide.addOneToManyLines(field);
					}
				});
			});
		}
		// 下载
		addBeansToZip(systeminfo.getSystemkey(), schemeName, classBeans);
	}

	private static final String IGNORE_TABLE_F = "f_";
	private static final String IGNORE_TABLE_FOV = "fov_";
	private static final String IGNORE_TABLE_ACT = "act_";

	public void addBeansToZip(String systemkey, String schemeName, Map<String, TableBeanGenerator> classBeans)
			throws IOException {
		...
		zipOut.setComment("这是数据库：" + schemeName + " 的bean文件");
		classBeans.forEach((tablename, classBean) -> {
			String tn = tablename.toLowerCase();
			if (!(tn.startsWith(IGNORE_TABLE_F) || tn.startsWith(IGNORE_TABLE_FOV)
					|| tn.startsWith(IGNORE_TABLE_ACT))) {
				try {
					input[0] = classBean.getInputStream();
					ZipEntry zipEntry = new ZipEntry(classBean.getBeanClassName() + ".java");
					zipEntry.setComment(tablename);
					zipOut.putNextEntry(zipEntry);
					int readed = 0;
					byte[] cash = new byte[2048];
					while ((readed = input[0].read(cash)) > 0) {
		...
		String filename = "数据库" + schemeName + "(" + systemkey + ")的bean文件" + ".zip";
	...
	/**
	 * 组织生成一个表的bean基本信息，包括表信息和所有字段，以及外键信息。
	 * 
	 * @param schemeName
	 * @param tableName
	 */
	public TableBean genTableBean(String schemeName, String tableName) {
		TableBean tableBean = dao.getSf().getTable(dao, tableName, schemeName);
		Connection connection = dao.getConnection();
		String sql = "select * from " + schemeName + (dao.isSqlserver() ? ".." : ".") + tableName + " where 1=2";
		try {
			...
			for (int i = 1; i < columeCount + 1; i++) {
				...
				Integer presicision = meta.getPrecision(i);
				Integer scale = meta.getScale(i);
				tableBean.getFields().forEach(field -> {
					if (field.getFieldname().equals(columnName)) {
						field.setDbfieldtype(columnType);
						field.setFieldtype(columnClassName.equals("[B") ? "byte[]" : columnClassName);
						field.setFieldlen(BigInteger.valueOf(presicision.longValue()));
						field.setDatascale(scale);
					}
				});
			}
