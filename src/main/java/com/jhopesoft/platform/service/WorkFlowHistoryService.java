...
public class WorkFlowHistoryService {
	...
	public ActionResult getHistoryInfo(String moduleName, String idvalue) {
		...
		if (processInstance.getEndTime() != null) {
			...
			HistoricActivityInstance instance = historyService.createHistoricActivityInstanceQuery()
					.processInstanceId(processInstance.getId()).activityId(processInstance.getEndActivityId())
					.singleResult();
			if (instance != null) {
				// 流程结束名称，正常结束，还是终止流程这样的文字
				jsonobject.put("endName", instance.getActivityName());
			}
		}

		JSONArray taskArray = new JSONArray();
		List<HistoricTaskInstance> list = historyService.createHistoricTaskInstanceQuery()
				.processDefinitionKey(object.getObjectname()).processInstanceBusinessKey(idvalue)
				.orderByTaskCreateTime().asc().list(); // 取得所有的任务.finished()
		for (HistoricTaskInstance hti : list) {
			...
			if (hti.getEndTime() != null) {
				// 没有 endtime 说明正在这个人这里需要审批
				taskObject.put("endTime", hti.getEndTime());
				String taskActionName = getTaskActionName(hti.getId());
				if (taskActionName != null) {
					taskObject.put("taskActionName", taskActionName);
					// 审批时写的备注
					taskObject.put("comment", getTaskComment(hti.getId(), taskActionName));
				}
			}
	...
	private String getTaskComment(String taskid, String type) {
		// 由于增加comment的时候，用的是有type类型的， 但是这里取不到_type, addComment 只能一条记录。
		String result = ""; 
