...
public class FormSchemeService {
	...
	public JSONObject getFormSchemeDetails(HttpServletRequest request, String formSchemeId) {
		...
		object.put(Constants.CHILDREN, genFormSchemedetail(details, baseModule));
		return object;
	}
	...
	private void saveNewDetails(FovFormscheme formScheme, JSONArray arrays, FovFormschemedetail p) {
		for (int i = 0; i < arrays.size(); i++) {
			...
			if (detailObject.containsKey("fieldahead")) {
				...
				dao.save(detail);
			} else if (detailObject.containsKey(Constants.CHILDREN)) {
	...
	public ActionResult checkNameValidate(String name, String id) {
		...
		for (FovFormscheme scheme : user.getFovFormschemes()) {
			boolean cond = (id == null || !id.equals(scheme.getFormschemeid())) && scheme.getSchemename().equals(name);
			if (cond) {
	...
	public ActionResult formSchemeSaveas(String schemeid, String schemename) {
		...
		for (FovFormschemedetail c : scheme.getDetails()) {
			FovFormschemedetail newdetail = new FovFormschemedetail(c.getFDataobjectBySubobjectid(),
					c.getFDataobjectfield(), c.getFDataobjectconditionBySubconditionid(), c.getOrderno(), c.getXtype(),
					c.getRegion(), c.getLayout(), c.getWidths(), c.getCols(), c.getRowss(), c.getRowspan(),
					c.getColspan(), c.getSeparatelabel(), c.getCollapsible(), c.getCollapsed(), c.getTitle(),
					c.getAggregate(), c.getFieldahead(), c.getWidth(), c.getHeight(), c.getHiddenlabel(),
					c.getIsendrow(), c.getOthersetting(), c.getRemark(), c.getSubobjectid(), c.isLeaf(),
					c.getShowdetailtip());
