...
public class UserRoleService {
			...
			if (group.getFCompanymodulegroups().size() > 0) {
				...
				for (FCompanymodule cmodule : group.getFCompanymodules()) {
					...
					if (object != null && object.getHasenable() != null && object.getHasenable()) {
						...
						subnode.setChildren(getCompanymoduleFunction(cmodule, checkedfunction, addall, subnode));
	...
	public List<TreeNode> getCompanymoduleFunction(FCompanymodule cmodule, Set<FModulefunction> checkedfunction,
			boolean addall, TreeNode parent) {
		List<TreeNode> result = new ArrayList<TreeNode>();
		FDataobject object = cmodule.getFModule().getFDataobject();
		// 显示在grid中的属性
		...
		for (FModulefunction cfunction : cmodule.getFModulefunctions()) {
			...
			if (cfunction.getFDataobjectbasefuncion() != null) {
				...
				if (basefunction.getFcode().indexOf(Constants.NEW) == 0) {
					if (!(object.getHasinsert() != null && object.getHasinsert())) {
						continue;
					}
				}
				if (basefunction.getFcode().indexOf(Constants.EDIT) == 0) {
					if (!(object.getHasedit() != null && object.getHasedit())) {
						continue;
					}
				}
				if (basefunction.getFcode().indexOf(Constants.DELETE) == 0) {
					if (!(object.getHasdelete() != null && object.getHasdelete())) {
						continue;
					}
				}
				if (basefunction.getFcode().indexOf(Constants.ATTACHMENT) >= 0) {
					if (!(object.getHasattachment() != null && object.getHasattachment())) {
						continue;
					}
				}
				if (basefunction.getFcode().indexOf(Constants.APPROVE) >= 0) {
					if (BooleanUtils.isNotTrue(object.getHasapprove())) {
						continue;
					}
				}
				if (basefunction.getFcode().indexOf(Constants.AUDITING) >= 0) {
					if (BooleanUtils.isNotTrue(object.getHasaudit())) {
						continue;
					}
				}
				// 直接修改，只有可审批和可审核的模块才有
				if (basefunction.getFcode().indexOf(Constants.DIRECTEDIT) == 0) {
					if (!(object.getHasedit() != null && object.getHasedit())) {
						continue;
					}
					if (!(BooleanUtils.isTrue(object.getHasapprove()) || BooleanUtils.isTrue(object.getHasaudit()))) {
						continue;
					}
				}
				TreeNode functionnode = new TreeNode(basefunction.getTitle());
				...
				if (checkedfunction.contains(cfunction)) {
					...
					attr.put(ADDITIONFUNCTION,
							(lastaddi == null ? "" : attr.get(ADDITIONFUNCTION).toString() + Constants.COMMA)
		...
		for (FModule module : af.getFDataobject().getFModules()) {
			for (FCompanymodule cmodule : module.getFCompanymodules()) {
				...
				if (!found) {
					...
					dao.save(function);
					msg.append(cmodule.getFCompany().getCompanyname() + ";<br/>");
				}
			}
		}
	...
	public ActionResult saveRoleLimit(String roleid, String ids) {
		...
		if (ids != null && ids.length() > 0) {
			for (String flid : ids.split(Constants.COMMA)) {
