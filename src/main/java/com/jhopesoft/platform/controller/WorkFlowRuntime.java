...
public class WorkFlowRuntime extends SqlMapperAdapter {
	...
	public ActionResult startProcessInstance(String objectName, String id, String name) {
		...
		try {
			result = workFlowRuntimeService.startProcessInstance(objectName, id, name, null);
	...
	public ResultBean completeProcessTask(String objectName, String id, String name, String taskId, String outgoingid,
		...
		try {
			result = workFlowRuntimeService.completeProcessTask(objectName, id, name, taskId, outgoingid, outgoingname,
					type, content, moduledata);
		} catch (WorkFlowException e) {
			...
			e.printStackTrace();
			result.setSuccess(false);
			result.setMessage(CommonUtils.getThrowableOriginalMessage(e));
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
			result.setMessage(e.getMessage());
	...
	@RequestMapping(value = "/unclaim.do")
	 ...
	 * 取得当前流程审批图，红色显示已完成区域
	 * 
	 * @param processInstanceId 流程id , 显示出来的质量不好
	 * @throws IOException
	 */
	@RequestMapping(value = "/getdiagram.do")
